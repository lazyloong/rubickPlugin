...
export class Pinyin extends Array<PinyinChild> {
  ...
  constructor(query: string) {
    super();
    const pinyinDict = SimplifiedDict as Record<string, string[]>;
    this.text = query;
    this.text.split('').forEach((p) => {
      const pinyin = pinyinDict[p] ?? [p];
      this.push({
        character: p,
        pinyin,
      });
  ...
  match<T extends Item>(query: string, item: T, smathCase = false): MatchData<T> | undefined {
  ...
  override concat(pinyin: Pinyin): Pinyin {
...
// 输出: [[1,3],[5,5],[7,8]]
function toRanges(arr: number[]): [number, number][] {
